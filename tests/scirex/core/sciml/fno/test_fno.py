...
class TestSpectralConv2d:
    ...
    def test_initialization(self, rng_key):
        ...
        for in_channels, out_channels, modes in test_configs:
            layer = SpectralConv2d(in_channels, out_channels, modes, key=rng_key)
            ...
            # Check weight shapes
            assert layer.real_weights.shape == (in_channels, out_channels, modes, modes)
            assert layer.imag_weights.shape == (in_channels, out_channels, modes, modes)

    def test_complex_mult2d(self, rng_key):
        """Test complex multiplication operation"""
        layer = SpectralConv2d(2, 2, 4, key=rng_key)

        # Test with simple input
        x_hat = jnp.ones((2, 4, 4), dtype=jnp.complex64)
        weights = jnp.ones((2, 2, 4, 4), dtype=jnp.complex64)

        result = layer.complex_mult2d(x_hat, weights)
        ...
        # Test with zeros
        x_hat = jnp.zeros((2, 4, 4), dtype=jnp.complex64)
        result = layer.complex_mult2d(x_hat, weights)
        assert jnp.allclose(result, jnp.zeros_like(result))
    ...
    def test_forward_pass(self, rng_key):
        ...
        for size in spatial_sizes:
            x = jax.random.normal(rng_key, (in_channels, size, size))
            output = layer(x)

            # Check output shape
            assert output.shape == (out_channels, size, size)
            # Check output is finite
            assert jnp.all(jnp.isfinite(output))
    ...
    def test_fourier_properties(self, rng_key):
        """Test that the layer preserves Fourier transform properties"""
        modes = 4
        layer = SpectralConv2d(1, 1, modes, key=rng_key)
        ...
        x = x[None, None, :]
        x = jnp.repeat(x, 1, axis=0)
        x = jnp.repeat(x, 1, axis=1)

        # Get output
        output = layer(x)

        # Verify output shape and realness
        assert output.shape == (1, spatial_size, spatial_size)
        # The output should be real since input is real
        output_ft = jnp.fft.rfftn(output[0])

        assert jnp.allclose(output_ft[modes:, modes:].imag, 0.0, atol=1e-6)

        assert jnp.allclose(output_ft[modes:, modes:].real, 0.0, atol=1e-6)


class TestFNOBlock2d:
    """Test suite for the FNOBlock2d"""
    ...
    def test_initialization(self, rng_key):
        ...
        block = FNOBlock2d(in_channels, out_channels, modes, jax.nn.gelu, key=rng_key)

        # Check components
        assert isinstance(block.spectral_conv, SpectralConv2d)
        assert isinstance(block.bypass_conv, eqx.nn.Conv2d)
        assert block.activation == jax.nn.gelu
    ...
    def test_different_activations(self, rng_key):
        ...
        x = jax.random.normal(rng_key, (2, 64, 64))

        for activation in activations:
            block = FNOBlock2d(2, 2, 16, activation, key=rng_key)
            output = block(x)
    ...
    def test_bypass_connection(self, rng_key):
        """Test that the bypass connection is working"""
        block = FNOBlock2d(2, 2, 16, lambda x: x, key=rng_key)
...
class TestFNO2d:
    """Test suite for the complete FNO2d model"""
    ...
    def test_forward_shapes(self, rng_key):
        ...
        spatial_sizes = [32, 64, 128]
        for size in spatial_sizes:
            x = jax.random.normal(rng_key, (2, size, size))
            output = model(x)
...
def test_end_to_end_training2d(rng_key, dummy_data):
    """Test complete training loop"""
    model = FNO2d(
        in_channels=2,
        out_channels=1,
