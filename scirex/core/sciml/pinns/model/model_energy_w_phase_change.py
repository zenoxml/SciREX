...
class DenseModel(tf.keras.Model):
    ...
    @tf.function
    def semismooth_function(self, theta, param_dict=None):
        """
        ...
        L = param_dict["L"]
        cf = param_dict["cf"]
        Chi_res = param_dict["Chi_res"]
        b = param_dict["b"]
        eta = param_dict["eta"]

        alpha_greater_than = cu * theta + L
        alpha_less_than = cf * theta + (
            (cu - cf)(
                Chi_res * theta + (((1 - Chi_res) * (tf.exp(b * theta) - 1.0)) / b)
            )
        )

        return tf.where(tf.less(theta, 0.0), alpha_greater_than, alpha_less_than)
    
    @tf.function
    def thermal_conductivity(self, theta, param_dict=None):
        """
        Implements the piecewise function:
        f(theta) = e^theta  for theta < 0
        f(theta) = theta+1  for theta >= 0
        ...
        Chi_l_greater_than = 1.0
        Chi_l_less_than = Chi_res + (1 - Chi_res) * tf.exp(b * theta)

        Chi_l = tf.where(tf.less(theta, 0.0), Chi_l_greater_than, Chi_l_less_than)

        ku = 1.0 / ((eta/kl) + ((1 - eta)/kr))
        kf = 1.0 / ((eta/ki) + ((1 - eta)/kr))

        k = 1 / (Chi_l/ku + (1 - Chi_l)/kf)      

        return k

    @tf.function
    def train_step(
        self, beta_boundary=10.0, beta_initial=100.0, params_dict=None
    ) -> dict:
        with tf.GradientTape(persistent=True) as training_tape:
            ...
            with tf.GradientTape(persistent=True) as gradient_tape:
                ...
                w = self.semismooth_function(theta, params_dict)
                k = self.thermal_conductivity(theta, params_dict)
            ...
            grad_theta_xx = gradient_tape.gradient(k*grad_theta_x, self.input_tensor)[
                :, 0:1
            ]
