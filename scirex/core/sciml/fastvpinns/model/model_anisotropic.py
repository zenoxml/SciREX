...
# Custom Model
class DenseModelAnisotropic(tf.keras.Model):
    ...
    def __init__(
        ...
        Returns:
            None
        """
        super(DenseModelAnisotropic, self).__init__()
    ...
    def train_step(
        ...
        # create copy of the bilinear parameters dictionary
        bilinear_params_dict = dict(bilinear_params_dict)
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Reshape Anisotropic Biliear Parameters [eps] into a matrix
            for key, value in bilinear_params_dict.items():
                bilinear_params_dict[key] = tf.reshape(
                    value, [self.n_cells, self.pre_multiplier_val.shape[-1]]
                )
