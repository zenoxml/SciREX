...
i_output_path = "output/poisson_Al_Aiso_Quad_Square_train"  # Output path
...
def rhs(x, y):
    ...
    return 4*y**2*(2*x - 3*y**2)*np.sin(x + y**2)*np.cos(x) + 12*y**2*np.cos(x)*np.cos(x + y**2) + 2*(-2*x + 3*y**2)*np.cos(x)*np.cos(x + y**2) + 2*(2*x - 3*y**2)*np.sin(2*x + y**2) + 2*np.sin(x)*np.sin(x + y**2) - 2*np.cos(x)*np.cos(x + y**2)
...
weights_file_path = output_folder / 'model_poisson_al_aiso_quad_square_weights.h5'
