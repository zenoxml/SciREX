...
i_n_cells_x = 8  # Number of cells in the x direction
i_n_cells_y = 8  # Number of cells in the y direction
i_n_boundary_points = 400  # Number of points on the boundary
i_output_path = "output/inverse/inverse_poisson_Cu_Iso_Square_train"  # Output path
...
i_n_sensor_points = 100  # Number of sensor points
...
def left_boundary(x, y):
    ...
    """
    val = np.sin(x) * np.tanh(x) * np.exp(-1.0 * (EPS / 30) * (x**2)) * (10)
    return val
...
def rhs(x, y):
    ...
    return (
        ...
        * (1)
        * (
            40.0 * X * (eps / 30) * (np.tanh(X) ** 2 - 1) * np.sin(X)
            - 40.0 * X * (eps / 30) * np.cos(X) * np.tanh(X)
            + 10 * (eps / 30) * (4.0 * X**2 * (eps / 30) - 2.0) * np.sin(X) * np.tanh(X)
            + 20 * (np.tanh(X) ** 2 - 1) * np.sin(X) * np.tanh(X)
            - 20 * (np.tanh(X) ** 2 - 1) * np.cos(X)
            - 10 * np.sin(X) * np.tanh(X)
        )
        * np.exp(-1.0 * X**2 * (eps / 30))
    )
...
for epoch in tqdm(range(i_num_epochs)):
    ...
    loss = model.train_step(beta=i_beta, bilinear_params_dict=None)
