...
i_output_path = "output/inverse/inverse_poisson_Al_Iso_Circle_train"  # Output path
...
i_n_sensor_points = 200  # Number of sensor points
...
i_dtype = tf.float32
i_activation = "tanh"
i_beta = 100  # Boundary Loss Penalty ( Adds more weight to the boundary loss)
...
def rhs(x, y):
    """
    This function will return the right-hand side of the PDE
    """
    return 3.884*EPS*(-x**2 - y**2 + A)*np.exp(-1.0*(x**2 + y**2)/A)/A**2
...
# # generate sensor points x and y coordinates
# points, sensor_values = datahandler.get_sensor_data(
#     exact_solution, i_n_sensor_points, i_mesh_generation_method
# )


# sample random sensor points in the interior of a unit circle with centre at 0,0
theta = np.random.uniform(0, 2 * np.pi, i_n_sensor_points)
r = np.sqrt(np.random.uniform(0, 1, i_n_sensor_points))
x = r * np.cos(theta)
y = r * np.sin(theta)
points = np.column_stack((x, y))
sensor_values = exact_solution(x, y)

points = tf.constant(points, dtype=i_dtype)
sensor_values = tf.constant(sensor_values, dtype=i_dtype)

points = tf.reshape(points, [i_n_sensor_points, 2])
sensor_values = tf.reshape(sensor_values, [i_n_sensor_points, 1])
...
inverse_param_array = []  # inverse parameters
...
for epoch in tqdm(range(i_num_epochs)):
    ...
    loss = model.train_step(beta=i_beta, beta_sensor=10, bilinear_params_dict=None)
    ...
    inverse_param_array.append(loss["inverse_params"]["eps"])
...
# 5. Print Inverse Params
inverse_param_array = np.array(inverse_param_array)
plt.figure(figsize=(6.4, 4.8), dpi=300)
plt.plot(inverse_param_array)
plt.hlines(EPS, 0, len(inverse_param_array), colors="r", linestyles="dashed")
plt.title("Inverse Parameters")
plt.xlabel("Epochs")
plt.ylabel("Inverse Parameters")
plt.tight_layout()
plt.savefig(str(output_folder / "inverse_params.png"))
plt.close()
