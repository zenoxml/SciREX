...
class HeatSolver(PDESolver):
    ...
    @staticmethod
    def solve(nx: int = 256, nt: int = 100, alpha: float = 0.01, 
              L: float = 1.0, T: float = 0.3) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        To solve heat equation with periodic boundary conditions
        ...
        r = alpha * dt / dx**2
        if r > 0.5:
            raise ValueError(f"Heat equation stability violated: r={r:.4f} > 0.5")
        
        for n in range(nt_actual - 1):
            u[n+1, 1:-1] = u[n, 1:-1] + r * (u[n, 2:] - 2*u[n, 1:-1] + u[n, :-2])
            # Periodic boundary
            u[n+1, 0] = u[n, 0] + r * (u[n, 1] - 2*u[n, 0] + u[n, -1])
            u[n+1, -1] = u[n, -1] + r * (u[n, 0] - 2*u[n, -1] + u[n, -2])
