...
gcn = GCN([node_vector_size, 100, 100, num_classes], [jnp.tanh] * 3, model_key)
...
training_set = jax.random.choice(
    key, num_nodes, (num_of_nodes_in_training_set,), replace=False
)
...
model = GCNModel(gcn, loss_fn, [training_accuracy, overall_accuracy])

gcn = model.fit(x, A, degree, Y, learning_rate=5e-2, num_iters=50)

# Results
final_node_embeddings = gcn(x, A, degree)
final_training_accuracy_percentage = 100.0 * training_accuracy(final_node_embeddings)
final_overall_accuracy_percentage = 100.0 * overall_accuracy(final_node_embeddings)
